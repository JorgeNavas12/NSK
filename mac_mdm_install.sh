...
if [ $# -lt $minimumArg ] && [ $idP_mode -eq 0 ]; then
   ...
   echo "Usage: jamfnsclientconfig.sh for email based, <dummy param 1> <dummy param 2>  <currentUsername> <tenant url> <AD domain name> <Org Key> [silent_mode] [enrollauthtoken=token] [enrollencryptiontoken=token]"
   echo "Usage: jamfnsclientconfig.sh for upn based, <dummy param 1> <dummy param 2>  <currentUsername> <Adonman url>  <Org Key> <upn> [preference_file_name] [silent_mode] [enrollauthtoken=token] [enrollencryptiontoken=token]"
   echo "Usage: jamfnsclientconfig.sh for peruserconfig mode , <dummy param 1> <dummy param 2>  <currentUsername> <Adonman url>  <Org Key> <peruserconfig> [fail-close fail-close-disable/fail-close-no-npa] [silent_mode] [enrollauthtoken=token] [enrollencryptiontoken=token]"
   echo "Usage: jamfnsclientconfig.sh for reading email from preference file mode , <dummy param 1> <dummy param 2> <dummy param 3> <tenant url> <Org Key> <preference_file_name> preference_email [silent_mode] [enrollauthtoken=token] [enrollencryptiontoken=token]"
   echo "Usage: jamfnsclientconfig.sh for reading logged in user using CLI tools, <dummy param 1> <dummy param 2> <dummy param 3> <tenant url> <Org Key> cli_mode [silent_mode]"
   echo "Usage: jamfnsclientconfig.sh for IdP mode , <dummy param 1> <dummy param 2>  <dummy param 3> idp <domain> <tenant> <requestEmail 1/0> [peruserconfig] [fail-close fail-close-disable/fail-close-no-npa] [silent_mode] [enrollauthtoken=token] [enrollencryptiontoken=token]"
   exit 1
fi
